...
export const POST: APIRoute = async ({ request, cookies, site }) => {
  ...
  if (error) {
    console.error(`[API /api/auth/register] Błąd rejestracji Supabase dla email: ${email}:`, error.message, `(Status: ${error.status})`);
  ...
  console.log(`[API /api/auth/register] Pomyślna próba rejestracji dla użytkownika: ${data.user?.email} (ID: ${data.user?.id}). Oczekiwanie na weryfikację email.`);

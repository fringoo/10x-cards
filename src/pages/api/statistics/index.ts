...
export const GET: APIRoute = async ({ locals }) => {
  ...
  if (authError || !user) {
    return new Response("Brak autoryzacji. Użytkownik musi być zalogowany, aby wyświetlić statystyki.", { status: 401 });
  }

  try {
    // Używamy Promise.allSettled, aby uzyskać jak najwięcej danych, nawet jeśli jedna z funkcji statystyk zawiedzie
    const results = await Promise.allSettled([
      getGlobalFlashcardStats(supabase),
      getGlobalEngagementStats(supabase),
    ]);
    ...
    const systemStats: SystemStatistics = {
        ...
        flashcardStats: flashcardStats || {
            totalFlashcards: 0, aiFlashcards: 0, manualFlashcards: 0, 
            aiFlashcardsAccepted: 0, aiAcceptanceRate: null
        },

...
const ResetPasswordForm: React.FC = () => {
  ...
  const [formMessage, setFormMessage] = useState<{ type: "success" | "error" | "default"; message: string } | null>(
    null
  );
  ...
  const handlePasswordRecovery = useCallback(() => {
    supabaseClient.auth.onAuthStateChange(async (event, session) => {
      ...
      if (event === "PASSWORD_RECOVERY") {
        console.log(
          "[ResetPasswordForm] Wykryto zdarzenie PASSWORD_RECOVERY. Formularz resetowania hasła jest aktywny."
        );
        ...
        console.log(
          "[ResetPasswordForm] Użytkownik zalogowany lub sesja zaktualizowana, ale nie w trybie PASSWORD_RECOVERY."
        );
      }
    });
  }, []);
  ...
  useEffect(() => {
    ...
    if (window.location.hash.includes("type=recovery")) {
      console.log(
        "[ResetPasswordForm] Wykryto fragment #type=recovery w URL. Oczekiwanie na zdarzenie PASSWORD_RECOVERY z Supabase SDK."
      );
      ...
      console.log(
        "[ResetPasswordForm] Brak fragmentu #type=recovery w URL. Formularz może nie być aktywny, jeśli Supabase nie zainicjuje sesji odzyskiwania."
      );
    }
  ...
  const onSubmit = async (data: ResetPasswordFormInputs) => {
    if (!canResetPassword) {
      ...
      setFormMessage({
        type: "error",
        message:
          "Nie można zresetować hasła. Sesja odzyskiwania nie jest aktywna. Spróbuj ponownie kliknąć link z emaila.",
      });
      return;
    }
      ...
      <Card className="w-full max-w-md text-center">
        ...
        <CardContent>
          <p className="text-sm text-muted-foreground">
            Jeśli przeszedłeś tutaj z prawidłowego linku do resetowania hasła, formularz powinien pojawić się za chwilę.
      ...
      <CardHeader>
        ...
        {!canResetPassword && formMessage?.type !== "success" && (
          <CardDescription className="text-red-500">
            Link do resetowania hasła jest nieprawidłowy, wygasł lub wystąpił problem z jego przetworzeniem. Spróbuj
            ponownie wygenerować link.
          </CardDescription>
        )}
      </CardHeader>
